...
BEGIN
  -- Verify caller is member of the work item's project
  IF NOT EXISTS (
    SELECT 1 FROM work_items wi
    WHERE wi.id = p_work_item_id
      AND public.is_project_member(auth.uid(), wi.project_id)
  ) THEN
    RAISE EXCEPTION 'Access denied';
  END IF;

  SELECT json_build_object(
    'work_item', (SELECT row_to_json(wi) FROM work_items wi WHERE wi.id = p_work_item_id),
    'ppi_instances', (
      SELECT COALESCE(json_agg(json_build_object(
        'code', pi.code,
        'template_name', COALESCE(pt.name, '—'),
        'status', pi.status,
        'items', (
          SELECT COALESCE(json_agg(json_build_object(
            'description', pii.label,
            'point_type', COALESCE(pii.inspection_point_type, 'RP'),
            'status', pii.result,
            'observation', pii.notes
          ) ORDER BY pii.sort_order, pii.item_no), '[]'::json)
          FROM ppi_instance_items pii
          WHERE pii.instance_id = pi.id
        )
      ) ORDER BY pi.created_at DESC), '[]'::json)
    ...
    'non_conformities', (
      ...
      WHERE nc.work_item_id = p_work_item_id AND nc.is_deleted = false
    ),
    'materials', (
      SELECT COALESCE(json_agg(json_build_object(
