...
export default function MonthlyReportPage() {
  ...
  if (selectedReport) {
    ...
    const kpis = [
      ...
      { label: t("monthlyReport.kpi.pameApproved", { defaultValue: "PAMEs aprovados F/IP" }), value: String(r.kpi_mat_approved ?? 0), ok: true },
      { label: t("monthlyReport.kpi.pamePending", { defaultValue: "PAMEs pendentes F/IP" }), value: String(r.kpi_mat_pending ?? 0), ok: (r.kpi_mat_pending ?? 0) === 0 },
      { label: t("monthlyReport.kpi.lotsReceived", { defaultValue: "Lotes recebidos (GR)" }), value: String((r as any).kpi_lots_received ?? 0), ok: true },
      { label: t("monthlyReport.kpi.lotsApproved", { defaultValue: "Lotes aprovados" }), value: String((r as any).kpi_lots_approved ?? 0), ok: true },
      { label: t("monthlyReport.kpi.lotsQuarantine", { defaultValue: "Lotes em quarentena" }), value: String((r as any).kpi_lots_quarantine ?? 0), ok: ((r as any).kpi_lots_quarantine ?? 0) === 0 },
                 ...
                 try {
                   ...
                   // Direct update via supabase since service type is narrow
                    await (supabase as any).from("monthly_quality_reports").update({
                      ...
                      kpi_pame_total: kpis.kpi_pame_total,
                      kpi_pame_approved: kpis.kpi_pame_approved,
                      kpi_lots_received: kpis.kpi_lots_received,
                      kpi_lots_approved: kpis.kpi_lots_approved,
                      kpi_lots_quarantine: kpis.kpi_lots_quarantine,
                      kpi_lots_rejected: kpis.kpi_lots_rejected,
