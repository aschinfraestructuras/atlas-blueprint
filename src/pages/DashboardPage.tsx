...
export default function DashboardPage() {
  ...
  const modules = [
    { icon: AlertTriangle,  label: t("dashboard.module.nc",        { defaultValue: "Não Conformidades" }), value: kpis.ncOpen,        total: undefined,         status: ncStatus,    sub: kpis.ncOpen > 0 ? `${kpis.ncOpen} em aberto` : "Sem alertas",         route: "/non-conformities" },
    { icon: ClipboardCheck, label: t("dashboard.module.ppi",       { defaultValue: "Inspecções PPI" }),    value: kpis.ppiApproved,   total: kpis.ppiTotal,     status: ppiStatus,   sub: `${ppiPct}% aprovados`,                                                route: "/ppi" },
    { icon: FlaskConical,   label: t("dashboard.module.tests",     { defaultValue: "Ensaios" }),           value: kpis.testsCompleted,total: kpis.testsTotal,   status: testsStatus, sub: `${testsPct}% realizados`,                                             route: "/tests" },
        ...
        <TabsContent value="overview" className="space-y-5 mt-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <AnimatedKpiCard icon={Zap}            label={t("dashboard.kpi.weldsPendingUtShort",{ defaultValue: "Soldaduras s/US" })} value={kpis.weldsPendingUt}      sub={kpis.weldsPendingUt===0?"Tudo inspeccionado":"Pendentes"}     color={kpis.weldsPendingUt>0?"hsl(0 65% 50%)":"hsl(145 55% 42%)"} route="/tests" loading={kpiLoading} delay={240} />
            </div>
          </div>
        </TabsContent>
