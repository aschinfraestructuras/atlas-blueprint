...
import { topographyControlService, type TopographyEquipment } from "@/lib/services/topographyService";
...
export function ControlFormDialog({ open, onOpenChange, projectId, equipment, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await topographyControlService.create({
      ...
      // The DB trigger will also block this – show its message
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Novo Controlo Geométrico</DialogTitle>
        ...
        <div className="grid gap-4">
          ...
          <Button onClick={handleSubmit} disabled={loading || !!isCalibrationInvalid}>
            {loading ? "A guardar…" : "Registar Controlo"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
